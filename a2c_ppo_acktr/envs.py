...
def make_vec_envs(env_name,
    ...
    if not obs_shape:
        is_robot = 1
        obs_shape = [envs.observation_space.spaces["observation"].shape[0]]
    ...
    if is_robot :
        envs = VecPyTorchRobotics(envs, device)
    else:
...
class VecPyTorchRobotics(VecEnvWrapper):
    def __init__(self, venv, device):
        """Return only every `skip`-th frame"""
        super(VecPyTorchRobotics, self).__init__(venv)
        self.device = device
        # TODO: Fix data types

    def reset(self):
        obs = self.venv.reset()
        obs = torch.from_numpy(obs['observation']).float().to(self.device)
        return obs
